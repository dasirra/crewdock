...
- A [Claude Code OAuth token](https://claude.ai/code) (`CLAUDE_CODE_OAUTH_TOKEN`)
- A [GitHub personal access token](https://github.com/settings/tokens) (`GH_TOKEN`) — for Forge
...
Edit `home/.openclaw/workspace/agents/forge/config.json` with your GitHub repos:
...
| `GH_TOKEN` | Yes | GitHub PAT for Forge and `gh` CLI |
...
4. Configure the gateway to use Tailscale Serve. In `home/.openclaw/openclaw.json`, set:
