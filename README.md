...
| `OPENCLAW_GATEWAY_TOKEN` | Recommended | Gateway auth token. Set this for a stable token that persists across container rebuilds. If set, `11-secrets.sh` migrates it to a SecretRef on first boot so it is no longer stored as plaintext. If not set, a random token is generated but not migrated. |
